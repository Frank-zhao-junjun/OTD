import { escapeODataString } from '@/lib/utils';
...
export async function fetchMaterialDocumentHeaderDates(
  ...
  const filter = unique
    .map((d) => `(MaterialDocument eq '${escapeODataString(d.MaterialDocument)}' and MaterialDocumentYear eq '${escapeODataString(d.MaterialDocumentYear)}')`)
    .join(' or ');
